...
# class_loc tells function what column class is located in the data
def defaultRate(input_df, class_cnt, class_loc):
    np_df = np.copy(input_df)
    row_size = input_df.shape[0]
    cnt = [0]*class_cnt

    for i in range(0,row_size):
        cur_class = np_df[i][class_loc]
        for k in range(0,class_cnt):
            if k+1 == cur_class:
                cnt[k]+=1
    
    # find most common class's size
    max = 0
...
def defaultRateIris(input_df):
    class_cnt = 3
    class_loc = 4
    np_df = np.copy(input_df)
    row_size = input_df.shape[0]
    cnt = [[0]*2 for i in range(class_cnt)]
    cnt[0][0] = 'Iris-setosa'
    cnt[1][0] = 'Iris-versicolor'
    cnt[2][0] = 'Iris-virginica'

    for i in range(0,row_size):
        cur_class = np_df[i][class_loc]
        for k in range(0,class_cnt):
            if cnt[k][0] == cur_class:
                cnt[k][1]+=1
...
# For reading iris data
def findAccuracyIris(input_df, feature_subset):
    row_size = input_df.shape[0]
    col_size = input_df.shape[1]
    correct_cnt = 0

    # Remove features not in subset (iris data has class at the end)
    np_df = np.copy(input_df)
    ...
    for i in range(0,row_size):
        cur_class = np_df[i][col_size-1]
        nn_dist = None
        nn_class = None

        cur_instance = np_df[i, 0:col_size-1]
        
        # test one data instance, i, against all others
        # find distance from current instance to all other rows
        for j in range(0,row_size):
            if(j!=i):
                dist = np.linalg.norm(cur_instance - np_df[j, 0:col_size-1])
                if not nn_dist or dist < nn_dist:
                    nn_dist = dist
                    nn_class = np_df[j][col_size-1]
...
# df = df.to_numpy()
# feature_subset = {0,1,2,3} # EXPECTED VAL: 0.963

# start = time.time()

# print(defaultRateIris(df))
# print("Execution Time: ", time.time()-start)